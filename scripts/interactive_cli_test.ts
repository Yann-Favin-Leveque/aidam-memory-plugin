...
interface TestStep {
  action: "send" | "wait_for" | "sleep" | "check_db" | "check_log" | "snapshot";
  /** For "send": text to type (\\n appended automatically) */
  text?: string;
  /** For "wait_for": regex pattern to match in output */
  pattern?: string;
  /** For "sleep": milliseconds to wait */
  ms?: number;
  /** For "check_db": SQL query */
  sql?: string;
  /** For "check_db": expected pattern in result */
  expect?: string;
  /** For "check_log": file path to check */
  file?: string;
  /** Timeout in ms for wait_for (default 30000) */
  timeout?: number;
...
class InteractiveCLI {
  ...
  async waitFor(pattern: string, timeoutMs: number = 30000): Promise<string> {
  ...
  async waitForNew(pattern: string, timeoutMs: number = 30000): Promise<string> {
    const regex = new RegExp(pattern, "i");
    const start = Date.now();
  ...
  getTail(chars: number = 2000): string {
...
async function runTestScript(script: TestScript): Promise<StepResult[]> {
...
function getAidamPluginTest(): TestScript {
  return {
...
function getAidamInteractiveTest(): TestScript {
  return {
