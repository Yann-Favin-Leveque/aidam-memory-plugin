...
class Orchestrator {
    ...
    async start() {
        ...
        await this.db.query(`INSERT INTO orchestrator_state (session_id, pid, retriever_enabled, learner_enabled, status)
       ...
       WHERE session_id = $1`, [this.config.sessionId, this.retrieverSessionId || null, this.learnerSessionId || null]);
        this.running = true;
        this.startPolling();
        this.startHeartbeat();
        if (this.config.compactorEnabled) {
...
Search memory for relevant context for this user's work. If nothing relevant, respond with SKIP.`;
    ...
    async writeRetrievalResult(promptHash, type, text) {
        await this.db.query(`INSERT INTO retrieval_inbox (session_id, prompt_hash, context_type, context_text, relevance_score)
       VALUES ($1, $2, $3, $4, $5)`, [this.config.sessionId, promptHash, type, text, text ? 0.8 : 0.0]);
    }
    // ============================================
    // LEARNER ROUTING
    // ============================================
...
Analyze this tool call. If it contains a valuable learning, error solution, or reusable pattern, save it to memory (check for duplicates first). If trivial, respond with SKIP.`;
    ...
    async runCompactor(transcriptPath, currentTokenEstimate) {
